...
test_that("`.fitEC()` returns expected results multiple covariates", {
  ...
  expect_equal(
    withr::with_seed(345, 
                     .fitEC(data.rct, data.ec, outcome.type = "cont", 
                            caret::trainControl(method = 'cv', number = 10L), "lm")),
               data.ec)
  
  
  expect_no_warning(suppressMessages(
    .fitEC(data.rct, data.ec, 
           caret::trainControl(method = 'cv', number = 10L), "gbm", 
           verbose = FALSE, outcome.type = "cont")))
...
test_that("`.fitEC()` returns expected results intercept only models", {
  ...
  expect_equal(withr::with_seed(
    ...
    .fitEC(data.rct, data.ec2, caret::trainControl(method = 'cv', number = 10L), "lm", outcome.type = "cont"))),
               data.ec)
...
test_that("`.fitRCTOutcome()` returns expected results multiple covariates", {
  ...
  test <- withr::with_seed(345, 
                           .fitRCTOutcome(data.rct2, caret::trainControl(), "lm", outcome.type = "cont"))
...
test_that("`.fitRCTOutcome()` returns expected results intercept only models", {
  ...
  test <- withr::with_seed(345, 
                           .fitRCTOutcome(data.rct2, caret::trainControl(method = 'cv', number = 10L), "lm", outcome.type = "cont"))
  test$fit.Y$call <- NULL
  test$fit.Y$times <- NULL
  test$fit.Y$terms <- NULL
...
test_that("`.fitModel()` returns expected results", {
  ...
  withr::with_seed(
    123, {
      ...
      data.rct <- .fitRCTOutcome (data.rct = data.rct, 
                                  ...
                                  method = "lm", outcome.type = "cont")
      
      data.rct <- .fitRCTPropensity(data.rct = data.rct, 
                                    ps.rct = NULL)

      tmp <- .fitEC(data.rct = data.rct, data.ec = data.ec,
                        ec.trControl = caret::trainControl(), method = "lm", outcome.type = "cont")
  ...
  tst <- withr::with_seed(123,
                          .fitModels(data.rct2, data.ec2, NULL, caret::trainControl(), caret::trainControl(), "lm", outcome.type = "cont"))
  data.rct$fit.Y <- NULL

...
#' 
#' @keywords internal
.lamdaHat <- function(data.rct, data.ec) {
  ...
  if (p_main_effect < n_ec) {
    ...
    lambda_hat <- tryCatch(BB::dfsane(par = rep(0.0, p_main_effect),
                                      fn = .calEqs,
                                      ...
                                      control = list(trace = TRUE,
                                                     ...
                                                     maxit = 500L)),
                           error = function(e) {
                             stop("error encountered in BB::dfsane\n\t",
                                  e$message, call. = FALSE)
                           })
    if (lambda_hat$convergence != 0) {
      warning("BB::dfsane returned a non-0 convergence flag: ", 
...
#' Intercept, mainName
.influenceFunction(data.rct, data.ec, lambda.hat, r.X, zero.bias) {
  ...
  stopifnot(
    ...
    "`lambda.hat` must be a numeric vector of lenght p_main_effects" = 
      !missing(lambda.hat) && is.vector(lambda.hat, "numeric") &&
      length(lambda.hat) == length(data.rct$mainName) + 1L,
    "`r.X` must be a positive scalar numeric" = !missing(r.X) &&
      is.vector(r.X, "numeric") && length(r.X) == 1L && r.X >= 0.0
    "`zero.bias` must be a logical vector" = !missing(zero.bias) &&
      is.vector(zero.bias, "logical") && length(zero.bias) == length(data.ec$Y)
  )
  ...
  # {n_rct + n_ec x p_me}
  S_acw_q <- rbind(-ME_rct_with_intercept, 
                    ME_ec_with_intercept * q_hat_ec)
  # {p_me x p_me}
  dot_S_acw_q <- t(q_hat_ec * ME_ec_with_intercept) %*% ME_ec_with_intercept / n_rct
  ...
  # STH verify that this is supposed to n_rct (might be n_ec)
  ...
  # {n_rct + n_ec}
  mu0_score <-  mu0_i - drop(S_acw_q %*% t({dot_mu0_rct_q - dot_mu0_ec_q} %*% inv_inf))
...
.estimateEC <- function(data.rct, data.ec, bias = NULL) {
  ...
  stopifnot(
    "`data.rct` must be a list containing elements {A, X, Y, mainName, ps, Y.hat.A0}" =
      !missing(data.rct) && is.vector(data.rct, "list") &&
      all (c("A", "X", "Y", "mainName", "ps", "Y.hat.A0") %in% names(data.rct))
    "`data.ec` must be a list containing elements {X, Y, mainName, ps, Y.hat}" =
      !missing(data.ec) && is.vector(data.ec, "list") &&
      all (c("A", "X", "Y", "mainName", "ps", "Y.hat") %in% names(data.ec)),
    ...
    "`bias` must be NULL or a numeric vector" = is.null(bias) || 
      {is.vector(bias, "numeric") && length(bias) == length(data.ec$Y)}
  )
  ...
  tmp <- if (any(zero_bias)) {
    .var(data.ec$Y.hat$ec[zero_bias])
  } else {
    1e6 
  }
  
  r_X <- .var(data.rct$Y.hat.A0[data.rct$A == 0L]) / tmp
  
  # construct the pseudo-observations for the bias parameters
  # {n_ec}
  bias <- data.ec$Y - data.ec$Yhat$rct
  ...
  # {n_ec}
  gamma_hat <- data.ec$Y.hat$ec - data.ec$Y.hat$rct
